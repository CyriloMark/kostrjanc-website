...
import { getCurrentLang, getPersistantAttributes, formularSections } from "../constants";
...
const Formular = () => {
    ...
    useEffect(() => {
        window.scrollTo(0, 0);
        document.title = getCurrentLang() === "sb" ? "kostrjanc - Formular" : "kostrjanc - Formular";
        if (window.location.hash) setActiveSection(window.location.hash.split("#")[1]);
    }, []);
    ...
    return (
        <div className={`bg-primary ${styles.flexCenterVert} flex-col w-full`}>
            ...
            <div className={`${styles.paddingX} ${styles.flexCenterVert} ${styles.boxWidth} flex-col relative`}>
                ...
                <SectionSelect active={activeSection} onSectionChange={(section) => {
                    setActiveSection(section);
                }} />

                {
                    activeSection === "business" ? <Business onLoad={onSectionLoad}
                        sectionInfo={formularSections.find(sec => sec.id === activeSection)} /> : null
                }
                {
                    activeSection === "werifikacija" ? <Werifikacija onLoad={onSectionLoad}
                        sectionInfo={formularSections.find(sec => sec.id === activeSection)} /> : null
                }
                {
                    activeSection === "ban" ? <Ban onLoad={onSectionLoad}
                        sectionInfo={formularSections.find(sec => sec.id === activeSection)} /> : null
                }
                {
                    activeSection === "hack" ? <Hack onLoad={onSectionLoad}
                        sectionInfo={formularSections.find(sec => sec.id === activeSection)} /> : null
                }
                {
                    activeSection === "bugs" ? <Bugs onLoad={onSectionLoad}
                        sectionInfo={formularSections.find(sec => sec.id === activeSection)} /> : null
                }
                {
                    activeSection === "feedback" ? <Feedback onLoad={onSectionLoad}
                        sectionInfo={formularSections.find(sec => sec.id === activeSection)} /> : null
                }
                <Footer />
            </div>
...
export const LinkToFormular = ({ styles, text, section }) => {
    return (
        <button type='button' className={`border-secondary border-[1px] rounded-lg py-2 px-3 font-barlow font-medium
            text-[18px] text-gradient transition-all duration-300 hover:rounded-none ${styles}`}>
            <Link to={"/pomoc/formular?" + getPersistantAttributes() + (section ? ("#" + section) : "")}>{text}</Link>
        </button>
    );
}