...
const AgbLegal = () => {
  useEffect(() => {
    ...
    document.title =
      getCurrentLang() === "sb"
        ? "kostrjanc - Škit datow / Prawniske"
        : "kostrjanc - Datenschutz / Rechliches";
  }, []);
  ...
  return (
    <div className={`bg-primary ${styles.flexCenterVert} flex-col w-full`}>
      ...
      <div
        className={`z-[5] ${styles.paddingX} ${styles.flexCenterVert} ${styles.boxWidth} flex-col`}
      >
        <div
          className={`${styles.paddingY} ${styles.flexStart} flex w-full flex-col`}
        >
          {privacySections.map((section, index) => (
            <div
              key={index}
              className={`flex flex-col w-full ${
                index === privacySections.length - 1 ? "mb-0" : "mb-5"
              }`}
            >
              ...
              {section.links
                ? section.links.map((link, ind) => (
                    <a
                      key={ind}
                      className={`${styles.paragraph} text-gradient cursor-pointer`}
                      href={link}
                    >
                      {link}
                    </a>
        ...
        <div
          className={`${styles.flexStart} ${styles.paragraph} self-start w-full max-w-[720px]`}
        >
          {getCurrentLang() === "sb"
