...
const FeatureCard = ({ icon, title, content, index }) => (
    <div className={`flex flex-row p-6 rounded-[20px] ${index !== business_features.length - 1 ? "mb-6" : "mb-0"} feature-card duration-300 w-full`}>
...
const Business = () => {
    return (
        <section id="business" className={`flex ${styles.paddingY} ${styles.boxWidth}`}>
            <div className={`flex-1 ${styles.flexStart} flex-col`}>
                {
                    getCurrentLang() === "sb" ?
                        <h2 className={styles.heading2}>Ty chceš našim <br className="md:block hidden" /><span className="text-gradient">Business</span> přistupić?</h2> :
                ...
                <div className={`flex-1 flex ${styles.flexCenter} ${styles.marginY} relative flex-col`}>
                    {
                        business_features.map((feature, index) => (
                            <FeatureCard key={feature.id} icon={feature.icon}
                                title={getCurrentLang() === "sb" ? feature.title : feature.title_de}
                                content={getCurrentLang() === "sb" ? feature.content : feature.content_de}
                                index={index} />
                        ))
                    }
                </div>
