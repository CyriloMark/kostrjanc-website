    ...
    return (
        <div id={contact.cont.id} className={`p-5`}>
            <div className={`flex-col flex ${styles.paragraph}`}>
                ...
                <a href={"mailto:" + contact.cont.email} className="text-gradient cursor-pointer">
                    {contact.cont.email}
                </a>
                <a href={"tel:" + contact.cont.tel} className="text-gradient cursor-pointer">
                    {contact.cont.tel}
                </a>
                <div className={`flex flex-col ml-5`}>
                    {
                        contact.cont.adress.map((txt, ind) => (
                            <p key={ind} className={`${ind === contact.cont.adress.length - 1 ? "mr-0" : "mr-5"}`}>{txt}</p>
                        ))
                    }
                </div>
            </div>
