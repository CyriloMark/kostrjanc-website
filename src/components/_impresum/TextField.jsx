...
const TextField = () => {
    ...
    let send = () => {
        ...
        window.open('mailto:info@kostrjanc.de?subject=' + message.subject + '&body=' + message.text, '_black').close();
    }

    return (
        <section id="textfiel" className={`flex flex-col ${styles.paddingY} ${styles.boxWidth}`}>
            {
                getCurrentLang() === "sb" ?
                    <h2 className={styles.heading2}>Maš wosebitu naležnosć?<br className="md:block hidden" />
                        Pisaj twoju powěsć hnydom tu!</h2> :
