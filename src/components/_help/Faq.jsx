...
const Faq = () => {
    ...
    return (
        <section id="faq" className={`${styles.boxWidth} ${styles.flexCenter} flex-col`}>
            <div className={`z-[5] w-full`}>
                {
                    faq.map((el, key) => (
                        <div key={key}
                            ...
                            onClick={() => openedSection === key ? setOpenedSection(-1) : setOpenedSection(key)} >
                            <p className={styles.paragraph}>{getCurrentLang() === "sb" ? el.title : el.title_de}</p>
                            {
                                openedSection === key ?
                                    el.answer
                                    : null
