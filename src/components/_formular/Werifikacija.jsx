...
const Werifikacija = ({ onLoad, sectionInfo }) => {
  ...
  const sendMail = () => {
    ...
    send({
      ...
      body: `Werifikacijske požadanje za kostrjanc%0AInformacije nic změnić! Jenož wotpósłać!%0A%0DMjeno, předmjeno: ${
        message.name
  ...
  return (
    <section
      id="werifikacija"
      className={`${styles.boxWidth} ${styles.paddingY}`}
    >
        ...
        <div className={`flex flex-row ${styles.flexCenter} my-2`}>
          ...
          >
            <img
              ...
              onClick={(prev) => {
                if (subjectToggle)
                  setMessage({ ...message, subject: "werifikacija" });
